...
abstract public class NetworkInterface implements ModuleCommunicationListener {
	/** interface name, e.g., bluetooth, DSRC -setting id ({@value})*/
	...
	public NetworkInterface(Settings s) {
		this.name = s.contains(NAME) ? s.getSetting(NAME) : null;
	...
	public void setHost(DTNHost host) {
		...
		if (transmitRange > 0) {
			...
			optimizer.setInterfaceName(this.name);
		} else {
			optimizer = null;
		}
	}
	 ...
	 */
	protected void notifyConnectionListeners(int type, DTNHost otherHost) {
